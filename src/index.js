...
export default class ModelMongo {
  ...
  _createCreateMutation = modelType => {
    ...
    this.Mutation._fields[name] = {
      ...
      name,
      resolve: async (parent, args, context) => {
        Object.entries(modelType._fields).forEach(([fieldName, field]) => {
          if (field.mmTransformAlways && field.mmTransformAlways.includes(KIND.CREATE)) {
            if (field.mmTransform) {
              args.data[fieldName] = args.data[fieldName] || null;
            }
          }
        });
        let doc = await applyInputTransform({parent, context})(args.data, inputType);
        if (typeWrap.isInherited()) {
          doc[
            typeWrap.interfaceType().mmDiscriminatorField
            ] = typeWrap.realType().mmDiscriminator;
  ...
  _createUpdateMutation = modelType => {
    ...
    this.Mutation._fields[name] = {
      ...
      resolve: async (parent, args, context) => {
        Object.entries(modelType._fields).forEach(([fieldName, field]) => {
          if (field.mmTransformAlways && field.mmTransformAlways.includes(KIND.UPDATE)) {
            if (field.mmTransform) {
              args.data[fieldName] = args.data[fieldName] || null;
            }
          }
        ...
        let {doc, validations, arrayFilters} = prepareUpdateDoc(
          await applyInputTransform({parent, context})(args.data, updateType)
        );
        let selector = {
