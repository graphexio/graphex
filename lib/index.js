...
var ModelMongo = function ModelMongo(_ref) {
  ...
  (0, _defineProperty2.default)(this, "_createConnectionQuery", function (modelType) {
    ...
    var connectionTypeName = "".concat(modelType.name, "Connection");
    var name = "".concat((0, _pluralize.default)(modelType.name).toLowerCase(), "Connection");
    _this.Query._fields[name] = {
  ...
  function () {
    ...
    _regenerator.default.mark(function _callee9(params) {
      var _params$schemaDirecti, schemaDirectives, _params$directiveReso, directiveResolvers, _params$resolvers, resolvers, _params$typeDefs, typeDefs, modelParams, schema, SchemaTypes, Query, Mutation;
        ...
        while (1) {
          switch (_context9.prev = _context9.next) {
            case 0:
              ...
              schema = (0, _graphqlTools.makeExecutableSchema)(modelParams);
              SchemaTypes = schema._typeMap;
              ...
              _lodash.default.values(SchemaTypes).forEach(function (type) {
                ...
                if ((0, _utils.getDirective)(type, 'model') || typeWrap.isInherited() && (0, _utils.getDirective)(typeWrap.interfaceType(), 'model')) {
                  // if (isInterface) {
                  //   this._createAllQueryInterface(type);
                  //   this._createSingleQueryInterface(type);
                  //   this._createConnectionQuery(type);
                  //
                  //   this._createCreateInterfaceMutation(type);
                  //   this._createDeleteInterfaceMutation(type);
                  //   this._createUpdateInterfaceMutation(type);
                  // } else {
