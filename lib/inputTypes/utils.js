...
const applyInputTransform = (context, key = "_fields") => {
  return async (value, type) => {
    if (type instanceof _graphql.GraphQLList) {
      return await Promise.all(value.map(val => applyInputTransform(context, key)(val, type.ofType)));
    ...
    let result = {};
    await Promise.all(_lodash.default.keys(fields).map(async key => {
