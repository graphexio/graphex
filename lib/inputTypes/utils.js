  ...
  return async (value, type) => {
    ...
    await Promise.all(Object.keys(fields).map(async key => {
      ...
      if (field.mmTransformAlways) {
        val = await field.mmTransformAlways({
          [key]: val
        }, context);
      }
      ...
      if (val !== undefined) {
        ...
        val = field.mmTransform ? await field.mmTransform({
          [key]: val
        }, context) : {
          [key]: await applyInputTransform(context)(val, field.type)
        };
        Object.entries(val).forEach(([k, v]) => result[k] = _lodash.default.isObject(v) && !Array.isArray(v) ? _lodash.default.merge(result[k], v) : v);
      }
    }));
    return result;
  };
